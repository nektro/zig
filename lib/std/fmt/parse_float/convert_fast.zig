...
pub fn convertFast(comptime T: type, n: Number(T)) ?T {
    ...
    if (n.exponent <= info.max_exponent_fast_path) {
        // normal fast path
        value = @floatFromInt(n.mantissa);
        value = if (n.exponent < 0)
            value / fastPow10(T, @intCast(-n.exponent))
        else
            value * fastPow10(T, @intCast(n.exponent));
    } else {
        // disguised fast path
        const shift = n.exponent - info.max_exponent_fast_path;
        const mantissa = math.mul(MantissaT, n.mantissa, fastIntPow10(MantissaT, @intCast(shift))) catch return null;
